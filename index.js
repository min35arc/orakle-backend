...
const { debugHouses, dateToJD } = require('./ephemeris');
...
// デバッグ用: swe_houses の生の戻り値を確認
// GET /api/debug-lagna?lat=36.34&lon=140.45&year=2003&month=10&day=5&hour=11&minute=8
app.get('/api/debug-lagna', (req, res) => {
  const { lat=36.34, lon=140.45, year=2003, month=10, day=5, hour=11, minute=8 } = req.query;
  ...
  const result = debugHouses(jd, parseFloat(lat), parseFloat(lon));
  res.json(result);
});
